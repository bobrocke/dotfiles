...
function configPath(): string {
  return join(homedir(), ".config", "opencode", "opencode-model-router", "tiers.json");
}
...
function buildDelegationProtocol(cfg: RouterConfig): string {
  ...
  return [
    ...
    ``,
    `### DISPATCH MARKER — NOT REQUIRED`,
    `The UI already renders each \`Task(...)\` as "Fast Task", "Medium Task", or "Heavy Task" based on the subagent_type — no additional markers needed. Dispatch silently. Example:`,
    ``,
    `\`\`\``,
    `Task(subagent_type="fast", prompt="find all usages of useAuth in src/")`,
    `\`\`\``,
    ``,
    `### HARD ROUTING (non-negotiable)`,
    `- **Read-only work** (grep, glob, read, ls, lookup, count, git-info, doc-lookup, type-check, exists-check) → \`Task(subagent_type="fast", ...)\` even for a single file read or grep. Direct read-only tools are allowed only when the user explicitly asks to avoid delegation or when the information is already in context. Rationale: every tool-result token is billed at your tier rate; a read via @fast uses DeepSeek V4 Flash instead of Pro.`,
    ...
    ``,
    `### ROLE CONTRACT`,
    `The primary agent's job: decompose the user's request, dispatch subagents, synthesize their results, and answer the user. Keep orchestration-first posture: dispatch read-only exploration to @fast rather than running Grep/Read/Glob/Bash calls yourself. Read-only self-cap is 0 by default; use direct reads only for explicit no-delegation requests or already-present context.`,
    ``,
    `### @fast contract`,
...
const CLAUDE_ORCHESTRATOR_PREFIX = [
  ...
  "AT MOST 0 direct read-only tool calls per turn by default. If you need",
  "context, dispatch @fast first; direct reads are only for explicit no-delegation requests.",
  "",
...
const ModelRouterPlugin: Plugin = async (_ctx: PluginInput) => {
  ...
  // Map sessionID → tier info for tier-badge injection in system.transform.
  const sessionTierMap = new Map<string, { name: string; model: string }>();
  ...
  return {
    ...
    "chat.message": async (input: any, output: any) => {
      if (bypassed) return;
      // Guard: subagent sessions may not have a sessionID yet when
      // chat.message fires — skip processing until the session is ready.
      ...
      if (input.agent && tierNames.includes(input.agent)) {
        ...
        // Store tier info for badge injection in system.transform.
        const tiers = getActiveTiers(cfg);
        const tierConfig = tiers[tierName];
        if (tierConfig) {
          sessionTierMap.set(input.sessionID, {
            name: tierName,
            model: tierConfig.model,
          });
        }

        // Prepend a visible tier badge to the subagent's message content so the
        // subagent knows its tier and the user can see it in expanded subagent output.
        const shortModel =
          tierConfig?.model.split("/").pop() ?? tierConfig?.model ?? "unknown";
        const tierBadge = `[tier:${tierName} · ${shortModel}]`;
        if (output?.parts && Array.isArray(output.parts)) {
          (output.parts as any[]).unshift({ type: "text" as const, text: tierBadge });
        }
      }
    },
    ...
    "experimental.chat.system.transform": async (_input: any, output: any) => {
      ...
      // Instead, inject a tier identification badge as the first system message
      // so the subagent sees its tier assignment and the user sees it in logs.
      const sessionID = _input?.sessionID;
      if (sessionID && subagentSessionIDs.has(sessionID)) {
        const tierInfo = sessionTierMap.get(sessionID);
        if (tierInfo) {
          const shortModel =
            tierInfo.model.split("/").pop() ?? tierInfo.model;
          const badge = `[tier:${tierInfo.name} · model: ${shortModel}]`;
          output.system.unshift(badge);
        }
        return;
      }
      ...
      // Prepend orchestrator identification badge so the model knows its role
      // and the user sees it in any expanded system-prompt view.
      const shortOrchModel =
        orchestratorModel.split("/").pop() ?? orchestratorModel;
      output.system.unshift(
        `[orchestrator · preset: ${cfg.activePreset} · model: ${shortOrchModel}]`,
      );
    },
